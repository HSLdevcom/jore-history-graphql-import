...
import { FTP_USERNAME, FTP_PASSWORD, FTP_HOST, FTP_PORT, FTP_PATH, DEBUG } from '../constants'
...
export async function fetchExportFromFTP() {
  ...
  const faultyFiles = await getErrorFiles().then((files) => files.map((file) => file.filename))
  ...
  const zips = files.filter(({ name }) => name.endsWith('.zip') && !faultyFiles.includes(name))
  ...
  console.log(
    `Latest imported export is ${get(latestImported, 'filename')}${
      !get(latestImported, 'success', false) ? ', which FAILED.' : ''
    }`
  )
